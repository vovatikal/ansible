...
class ElasticLoadBalancerV2:
    ...
    def get_elb_attributes(self):
        ...
        try:
            elb_attributes = boto3_tag_list_to_ansible_dict(self.connection.describe_load_balancer_attributes(LoadBalancerArn=self.elb['LoadBalancerArn'])['Attributes'])
        except ClientError as e:
    ...
    def compare_subnets(self):
        ...
        if set(subnet_id_list) != set(self.subnets):
            return False
        else:
            return True
...
class ALBListenerRules:
    ...
    def __init__(self, connection, module, elb_arn, listener_rules, listener_port):
        ...
        self.current_listener = ALBListeners.get_elb_listener(connection, module, elb_arn, listener_port)
